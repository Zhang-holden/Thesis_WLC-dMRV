# 技术路线：基于区块链的可信全生命周期碳资产化架构 (WLC-dMRV) 用于 EV 充电基础设施
...
- **专业与地域定位**：基础设施资产管理 (IAM)，新西兰 (New Zealand)
- **核心研究对象**：电动汽车 (EV) 充电基础设施
- **核心痛点**：
  - **物理边界缺失 (Physical Boundary Gap)**：现有 LCA 研究大多止步于 Cradle-to-Gate，仅覆盖建材生产至施工阶段。EV 充电基础设施长达 15-20 年的服役期中，B2-B5 阶段的维保换件隐含碳、C1-C4 阶段的报废拆除排放、以及 D 阶段的回收循环经济效应大多未被纳入统一的碳追踪框架。换句话说，一只充电枪从"出生"到"退役"的全生命周期碳账本从未真正闭合。
...
- **RQ1 (全生命周期 dMRV 数据闭环的信度)**：能否通过区块链、物联网与动态 LCA 的融合，构建一个覆盖 EV 充电基础设施全生命周期 (Cradle-to-Grave) 的碳追踪数据闭环，并在数据完整性、链上可审计性与跨方可验证性三个维度上，提供比传统中心化 MRV 更强的可信基础？
...
| 动态数据 | 新西兰 Transpower 实时边缘电网碳强度 MEF (gCO₂/kWh)              | API → 预言机 → 智能合约       |

***

### Layer 3：动态 LCA 与智能合约引擎层 (dLCA & Smart Contract Engine)

**定位**：系统的"大脑"，包含三个自动化逻辑模块。
...
**定位**：本研究最亮眼的机制设计，解决传统碳信用的"漂绿"和"废弃阶段环境负债无人担保"问题。
...
$$E\_{\text{embodied}} = \sum\_{i} Q\_i \times EF\_i^{\text{EPD}}$$

其中：

- $Q\_i$ 为第 $i$ 种材料用量
- $EF\_i^{\text{EPD}}$ 为相应材料的 BRANZ EPD 排放因子

**2. OpEx 动态运营碳排放：**

$$E\_{\text{op}}(t) = \sum\_{d=1}^{t} \text{kWh}\_d \times \text{MEF}\_d$$
...
$$\text{Net Reduction}(t) = \sum\_{d=1}^{t} \text{kWh}_d \times (EF_{\text{ICE}} - \text{MEF}\_d)$$

其中：

- $EF\_{\text{ICE}}$ 为同里程燃油车排放基准线因子

**4. 碳偿付判定（"净正向触发点"判定）：**

$$\text{Carbon Balance}(t) = \text{Net Reduction}(t) - E\_{\text{embodied}} - E\_{\text{maintenance}}(t)$$
...
| **2** | Literature Review | 2.1 EV 充电基础设施 LCA 研究现状 → 2.2 区块链在碳追踪领域的应用 → 2.3 代币经济学与环境资产化交叉研究 → 2.4 研究空白总结与对比表                                                 |
